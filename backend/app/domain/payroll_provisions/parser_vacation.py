...
class VacationSnapshot:
    def __init__(
        ...
        *,
        ...
        self.company_cnpj_base = company_cnpj_base
        self.competence = competence
        self.cost_center_code = cost_center_code
        self.cost_center_name = cost_center_name
        self.total_saldo_vacation = total_saldo_vacation
        self.total_saldo_bonus = total_saldo_bonus
        self.total_saldo_fgts = total_saldo_fgts
        self.total_saldo_inss = total_saldo_inss
        self.total_saldo_terc = total_saldo_terc
        self.total_saldo_rat = total_saldo_rat


def parse_single_vacation_report(path: str | Path) -> list[VacationSnapshot]:
    ...
    snapshots: list[VacationSnapshot] = []
...
def build_snapshot_index(
    snapshots: list[VacationSnapshot],
) -> dict[tuple[str, str], VacationSnapshot]:
    index: dict[tuple[str, str], VacationSnapshot] = {}
...
def parse_vacation_reports(
    report_a_path: str | Path,
    report_b_path: str | Path,
) -> list[Provision13thResult]:
    snapshots_a = parse_single_vacation_report(report_a_path)
    snapshots_b = parse_single_vacation_report(report_b_path)
    ...
    for key in all_keys:
        ...
        prev_vacation_total = (
            (prev.total_saldo_vacation + prev.total_saldo_bonus) if prev else 0.0
        )
        curr_vacation_total = (
            (curr.total_saldo_vacation + curr.total_saldo_bonus) if curr else 0.0
        )
        ...
        results.append(
            Provision13thResult(
                ...
                entries=[
                    Provision13thEntry(
                        entry_code="PROVFERIAS",
                        entry_description=f"PROVFERIAS {competence_to_display(competence_current)}",
                        amount_previous=round(prev_vacation_total, 2),
                        amount_current=round(curr_vacation_total, 2),
                        amount_difference=round(
                            curr_vacation_total - prev_vacation_total, 2
                        ),
                    ),
                    Provision13thEntry(
                        entry_code="PROVFGTSFERIAS",
                        entry_description=f"PROVFGTSFERIAS {competence_to_display(competence_current)}",
                        amount_previous=round(prev_fgts, 2),
                        amount_current=round(curr_fgts, 2),
                        amount_difference=round(curr_fgts - prev_fgts, 2),
                    ),
                    Provision13thEntry(
                        entry_code="PROVINSSFERIAS",
                        entry_description=f"PROVINSSFERIAS {competence_to_display(competence_current)}",
